export const TOOLS_DESCRIPTION = {
  ...
  getCalendarTypesByEventDetails: `Lists ALL calendars linked to the authenticated user with their IDs and names. The "email" parameter is REQUIRED and must be taken from the conversation context (provided in the "User Email" section) - DO NOT use placeholder emails or ask the user for their email. Returns array of { calendarId, calendarName } objects containing the COMPLETE list of available calendars. Calendar names may be in Hebrew, English, or other languages. This tool MUST be called before selecting a calendar to ensure you have the full list of available calendar IDs to choose from.`,
