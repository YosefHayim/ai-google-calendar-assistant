...
import { STATUS_RESPONSE, type TokensProps } from "@/types";
import { reqResAsyncHandler } from "@/utils/async-handlers";
import { fetchCredentialsByEmail } from "@/utils/get-user-calendar-tokens";
import { getEvents, insertEvent as insertEventHandler, updateEvent as updateEventHandler, deleteEvent as deleteEventHandler } from "@/utils/handle-events";
...
const getAllFilteredEvents = reqResAsyncHandler(async (req, res) => {
  const r = await getEvents({ req, extra: req.query as Record<string, string> });
...
const createEvent = reqResAsyncHandler(async (req, res) => {
  const r = await insertEventHandler({ req, eventData: req.body, extra: { calendarId: req.body.calendarId, email: req.body.email } });
...
const updateEvent = reqResAsyncHandler(async (req, res) => {
  ...
  const r = await updateEventHandler({ req, eventData: { id: req.params.eventId, ...req.body } });
...
const deleteEvent = reqResAsyncHandler(async (req, res) => {
  ...
  const r = await deleteEventHandler({ req, eventData: { id: req.params.eventId } });
