...
const generateAuthGoogleUrl = reqResAsyncHandler(async (req: Request, res: Response) => {
  ...
  if (userEmail) {
    ...
    // First find the user
    const { data: user } = await SUPABASE.from("users").select("id").ilike("email", normalizedEmail).limit(1).maybeSingle();
    ...
    if (user) {
      ...
      forceConsent = !existingToken?.refresh_token || !existingToken?.is_valid;
    }
  }
  ...
  try {
    ...
    const userUpsertPayload = {
      ...
      display_name: googleUser.given_name ? `${googleUser.given_name} ${googleUser.family_name || ""}`.trim() : null,
      email_verified: true,
    ...
    const { data: userData, error: userError } = await SUPABASE.from("users").upsert(userUpsertPayload, { onConflict: "email" }).select("id").single();

    if (userError || !userData) {
      console.error("Supabase User Upsert Error:", userError);
      return sendR(res, STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to store user in database.", userError);
    }
    ...
    const { error: tokenError } = await SUPABASE.from("oauth_tokens").upsert(tokenUpsertPayload, { onConflict: "user_id,provider" });

    if (tokenError) {
      console.error("Supabase Token Save Error:", tokenError);
      return sendR(res, STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to store Google tokens in database.", tokenError);
    }

    if (tokens.refresh_token && tokens.access_token) {
      syncUserCalendarsAfterOAuth(userData.id, tokens.access_token, tokens.refresh_token);
    }
...
const deActivateUser = reqResAsyncHandler(async (req: Request, res: Response) => {
  ...
  // Find the user first
  const { data: user, error: userError } = await SUPABASE.from("users").select("id").ilike("email", email).limit(1).maybeSingle();
...
const getGoogleCalendarIntegrationStatus = reqResAsyncHandler(async (req: Request, res: Response) => {
  ...
  // Find the user
  const { data: user, error: userError } = await SUPABASE.from("users").select("id, created_at").ilike("email", normalizedEmail).limit(1).maybeSingle();

  if (userError) {
    return sendR(res, STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to fetch user.", userError);
  }
  ...
  if (tokenError) {
    return sendR(res, STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to fetch integration status.", tokenError);
  }
