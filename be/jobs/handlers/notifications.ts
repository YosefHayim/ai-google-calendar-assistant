...
export async function handleEventReminderJob(
  ...
  const now = new Date()
  const reminderWindowStart = new Date(now.getTime() + (REMINDER_MINUTES_BEFORE - 1) * MINUTES_TO_MS)
  const reminderWindowEnd = new Date(now.getTime() + (REMINDER_MINUTES_BEFORE + 1) * MINUTES_TO_MS)

  for (const telegramUser of telegramUsers) {
    try {
      const { data: tokens } = await userRepository.findUserWithGoogleTokens(telegramUser.users.email)

      if (!tokens) {
        continue
      }

      const calendar = await initUserSupabaseCalendarWithTokensAndUpdateTokens(tokens)
      ...
      for (const event of upcomingEvents) {
        ...
        const sendResult = await sendTelegramNotification(telegramUser.telegram_chat_id, message)

        if (sendResult.success) {
          result.sent++
        } else {
          result.failed++
          result.errors.push(`Failed for ${telegramUser.users.email}: ${sendResult.error}`)
        }
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Unknown error"
      result.errors.push(`Error for ${telegramUser.users.email}: ${errorMessage}`)
    }
  }
...
type BriefingPrefs = { enabled: boolean; channel: string; time: string; lastSentDate?: string }
...
async function sendDigestToUser(
  ...
  const { data: tokens } = await userRepository.findUserWithGoogleTokens(user.email)
  if (!tokens) {
  ...
  endOfDay.setHours(END_OF_DAY_HOURS, END_OF_DAY_MINUTES, END_OF_DAY_SECONDS, END_OF_DAY_MS)
  ...
  if (sendResult.success) {
    ...
    const updatedPrefs = { ...existingPrefs, daily_briefing: { ...dailyBriefing, lastSentDate: today } }
...
export async function handleDailyDigestJob(
  ...
  const { data: usersWithBriefing, error } = await SUPABASE
    .from("users")
  ...
  for (const user of usersWithBriefing || []) {
    ...
    const shouldSkip = !dailyBriefing?.enabled || dailyBriefing.channel !== "telegram"
    if (shouldSkip) {
      continue
    }
