...
export async function handleTokenRefreshCheck(
  ...
  try {
    ...
    const { data: expiringTokens, error: fetchError } = await SUPABASE.from(
      "oauth_tokens"
    )
      .select(
        "user_id, refresh_token, access_token, expires_at, token_type, id_token, scope, users!inner(email)"
      )
      .eq("provider", "google")
      .eq("is_valid", true)
    ...
    if (fetchError) {
      ...
      logger.error(
        `[Job ${job.id}] Error fetching expiring tokens:`,
        fetchError
      );
      throw fetchError;
    }
    ...
    if (!expiringTokens || expiringTokens.length === 0) {
      logger.info(
        `[Job ${job.id}] No tokens expiring within ${EXPIRY_THRESHOLD_HOURS} hours`
      );
      return result;
    }

    logger.info(
      `[Job ${job.id}] Found ${expiringTokens.length} tokens to refresh`
    );
