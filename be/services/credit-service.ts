...
export async function checkUserCredits(userId: string): Promise<CreditCheckResult> {
  ...
  if (error) {
    logger.error(`[CreditService] Error checking credits for user ${userId}`, error)
    return {
      hasCredits: false,
      creditsRemaining: 0,
      subscriptionId: null,
...
export async function deductCredit(
  ...
  try {
    if (source === "subscription") {
      ...
      const newUsed = (sub.ai_interactions_used || 0) + CREDIT_COST_PER_INTERACTION
      ...
      if (subUpdateError) {
        logger.error("[CreditService] Error deducting subscription credit:", subUpdateError)
        return { success: false, newBalance: 0, error: subUpdateError.message }
      }

      logger.info(`[CreditService] Deducted 1 subscription credit for user ${userId}`)
      return { success: true, newBalance: newUsed }
    }

    const { data: creditSub, error: creditFetchError } = await SUPABASE
      .from("subscriptions")
    ...
    if (creditUpdateError) {
      logger.error("[CreditService] Error deducting credit pack:", creditUpdateError)
      return { success: false, newBalance: currentCredits, error: creditUpdateError.message }
    }

    logger.info(`[CreditService] Deducted 1 credit for user ${userId}, remaining: ${newBalance}`)
...
export class CreditTransaction {
  ...
  async commit(): Promise<CreditDeductionResult> {
    ...
    if (!this.creditCheck) {
      return { success: false, newBalance: 0, error: "No credit check performed" }
    }
  ...
  rollback(): void {
    ...
    logger.info(`[CreditService] Transaction rolled back for user ${this.userId} (no credit deducted)`)
  }
