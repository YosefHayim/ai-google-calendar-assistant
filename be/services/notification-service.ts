...
export type NotificationType = "event_reminder" | "daily_digest" | "weekly_summary"
...
export async function getTelegramUsersForNotifications(): Promise<TelegramUser[]> {
  ...
  if (error) {
    logger.error("[NotificationService] Failed to fetch Telegram users:", error)
    return []
  }
...
export async function sendTelegramNotification(
  ...
  try {
    ...
    logger.error(`[NotificationService] Failed to send to chat ${chatId}:`, error)
