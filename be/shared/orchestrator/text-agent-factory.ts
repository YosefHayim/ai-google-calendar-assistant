...
import { createProviderFromProfile, isOpenAIProvider } from "@/shared/llm";
...
export function createTextAgent(
  options: CreateTextAgentOptions
): TextAgentConfig {
          ...
          for (const tc of response.toolCalls) {
            messages.push({
              ...
              content: JSON.stringify({
                error: "No email context available for tool execution",
              }),
              toolCallId: tc.id,
              name: tc.name,
            });
...
export async function runTextAgent(
  ...
  if (!config.provider) {
    throw new Error(
      "Non-OpenAI profile requires a provider, but none was created"
    );
  }
