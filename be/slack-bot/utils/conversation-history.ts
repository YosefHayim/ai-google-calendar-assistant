...
export const getTodayConversationState = async (
  slackUserId: string,
  teamId: string
): Promise<{ id: string; user_id: string; summary: string | null; updated_at: string; created_at: string } | null> => {
...
export const getOrCreateTodayContext = async (
  ...
  const existingConversation = await getTodayConversationState(slackUserId, teamId)
...
export const addMessageToContext = async (
  ...
  const { stateId, context, userId } = await getOrCreateTodayContext(slackUserId, teamId)
  ...
  if (shouldSummarize) {
    ...
    try {
      ...
      await SUPABASE
        .from("conversations")
        .update({ summary: context.summary, updated_at: new Date().toISOString() })
        .eq("id", stateId)
    } catch (error) {
