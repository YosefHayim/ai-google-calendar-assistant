...
export class SlackResponseBuilder {
  ...
  eventCard(event: {
    ...
    const fields: string[] = []
    if (event.date) fields.push(`📅 ${event.date}`)
    if (event.time) fields.push(`🕐 ${event.time}`)
    if (event.location) fields.push(`📍 ${event.location}`)
    if (event.calendar) fields.push(`📁 ${event.calendar}`)
