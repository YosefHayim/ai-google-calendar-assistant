...
export const buildAgentPromptWithContext = (
  ...
  parts.push("<platform>Telegram - Keep responses concise and mobile-friendly.</platform>")
...
${truncatedContext}
  ...
  if (result.length > MAX_PROMPT_LENGTH) {
    logger.warn(`Prompt exceeded ${MAX_PROMPT_LENGTH} chars (${result.length}), truncating context`)
