...
export const buildConfirmationPrompt = (
  ...
  const timestamp = new Date().toISOString();
  return `Current date and time is ${timestamp}. User ${email} CONFIRMED creation of event despite conflicts. Create the event now with these details: ${JSON.stringify(eventData)}`;
};
