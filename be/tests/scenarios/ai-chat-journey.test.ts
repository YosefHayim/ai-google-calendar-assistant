...
import { mockFn, createMockUser, testData } from "../test-utils"
...
describe("AI Chat Interaction Journey", () => {
  ...
  describe("Scenario 1: Starting a New Conversation", () => {
    ...
    it("should generate conversation title from first message", () => {
      const userMessage = "Schedule a meeting with the team tomorrow at 2pm"
  ...
  describe("Scenario 2: Processing User Messages", () => {
    it("should classify intent from user message", () => {
      const messages = [
        { input: "Create an event tomorrow at 3pm", expectedIntent: "create_event" },
        { input: "What's on my calendar today?", expectedIntent: "list_events" },
        { input: "Cancel my meeting with John", expectedIntent: "delete_event" },
        { input: "Move my dentist appointment to Friday", expectedIntent: "update_event" },
        { input: "Find a free slot next week", expectedIntent: "find_availability" },
      ]
    ...
    it("should handle ambiguous requests with clarification", () => {
      const ambiguousInput = "meeting tomorrow"

      const needsClarification = {
        ambiguous: true,
        missingFields: ["time", "attendees", "title"],
        clarificationPrompt: "What time would you like to schedule the meeting?",
      }
  ...
  describe("Scenario 4: Multi-Turn Conversations", () => {
    ...
    it("should allow referencing previous events", () => {
      ...
      const userReference = "the first one"
  ...
  describe("Scenario 5: Credit/Usage Tracking", () => {
    ...
    it("should warn user when approaching limit", () => {
      ...
      const usagePercent = (subscription.ai_interactions_used / subscription.ai_interactions_monthly) * 100
  ...
  describe("Scenario 6: Guardrails and Safety", () => {
    ...
    it("should block mass deletion requests", () => {
      ...
      const massDeletePatterns = [/delete all/i, /clear.*entire/i, /remove everything/i]

      dangerousRequests.forEach((request) => {
        const isMassDelete = massDeletePatterns.some((pattern) => pattern.test(request))
    ...
    it("should respect user privacy in responses", () => {
      ...
      const maskedDescription = eventWithSensitiveData.description.replace(ssnPattern, "***-**-****")
  ...
  describe("Scenario 7: Error Recovery", () => {
    ...
    it("should provide helpful error messages", () => {
      const errors = [
        { code: "CALENDAR_NOT_CONNECTED", friendly: "Please connect your Google Calendar first." },
        { code: "RATE_LIMIT_EXCEEDED", friendly: "Too many requests. Please wait a moment." },
        { code: "EVENT_NOT_FOUND", friendly: "I couldn't find that event in your calendar." },
        { code: "PERMISSION_DENIED", friendly: "I don't have permission to access that calendar." },
      ]
