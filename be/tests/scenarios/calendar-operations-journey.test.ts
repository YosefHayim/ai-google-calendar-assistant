...
import { mockFn, createMockRequest, createMockResponse } from "../test-utils"
...
describe("Calendar Operations Journey", () => {
  ...
  describe("Scenario 1: OAuth Connection Flow", () => {
    ...
    it("should refresh tokens when near expiry", async () => {
      const nearExpiryTokens = {
        ...mockTokenData,
        expiry_date: Date.now() + 300000, // 5 minutes left
      }
      const needsRefresh = nearExpiryTokens.expiry_date - Date.now() < 600000 // 10 min threshold
  ...
  describe("Scenario 3: Creating Events", () => {
    ...
    it("should create a recurring event", async () => {
      ...
      expect(result.data.recurrence).toContain("RRULE:FREQ=WEEKLY;BYDAY=MO,WE,FR")
    })
  })

  describe("Scenario 4: Quick Add with AI Parsing", () => {
    it("should parse natural language event description", () => {
      const input = "Meeting with John tomorrow at 3pm for 1 hour"
    ...
    it("should detect and resolve ambiguous times", () => {
      const input = "Call with team at 3"
    ...
    it("should extract location from description", () => {
      const input = "Lunch at Cafe Milano at 12pm"
  ...
  describe("Scenario 7: Event Deletion", () => {
    it("should delete a single event", async () => {
      ...
      const result = await mockCalendar.events.delete({
        calendarId: "primary",
        eventId: "event-123",
      })
  ...
  describe("Scenario 8: Rescheduling Suggestions", () => {
    ...
    it("should prefer morning slots for morning preference", () => {
      ...
      const preference = "morning"
      const bestSlot = availableSlots.reduce((best, slot) => (slot.score > best.score ? slot : best))
    ...
    it("should exclude weekends when requested", () => {
      ...
      const filtered = suggestions.filter((s) => s.dayOfWeek >= 1 && s.dayOfWeek <= 5)
