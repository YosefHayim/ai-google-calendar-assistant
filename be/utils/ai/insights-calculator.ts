...
export function calculateInsightsMetrics(
  ...
  const allDayEvents = events.filter((e) => e.start?.date && !e.start?.dateTime && e.status !== "cancelled")
  ...
  for (const event of sortedEvents) {
    ...
    if (!earliestStart || start.getHours() * 60 + start.getMinutes() < earliestStart.getHours() * 60 + earliestStart.getMinutes()) {
      earliestStart = start
    }
    if (!latestEnd || end.getHours() * 60 + end.getMinutes() > latestEnd.getHours() * 60 + latestEnd.getMinutes()) {
      latestEnd = end
    }
    ...
    if (!longestEvent || durationHours > longestEvent.hours) {
      longestEvent = { title: event.summary || "Untitled", hours: Math.round(durationHours * 10) / 10 }
    }
    if (!shortestEvent || durationMinutes < shortestEvent.minutes) {
      shortestEvent = { title: event.summary || "Untitled", minutes: Math.round(durationMinutes) }
    }
    ...
    const calendarId = (event as { calendarId?: string }).calendarId || "primary"
    const calendarInfo = calendarMap[calendarId] || { name: calendarId, color: "#6366f1" }
    ...
    if (previousEventEnd) {
      const gapMinutes = (start.getTime() - previousEventEnd.getTime()) / (1000 * 60)
  ...
  const dayHoursArray = Object.entries(dayHours).map(([date, hours]) => ({ date, hours }))
  const sortedDays = dayHoursArray.sort((a, b) => b.hours - a.hours)

  const busiestDay = sortedDays[0]
    ? { day: formatDayName(sortedDays[0].date), hours: Math.round(sortedDays[0].hours * 10) / 10 }
    : null
  ...
  const quietestDay =
    daysWithEvents.length > 0
      ? {
          day: formatDayName(daysWithEvents[daysWithEvents.length - 1].date),
          hours: Math.round(daysWithEvents[daysWithEvents.length - 1].hours * 10) / 10,
        }
      : null
  ...
  const lastEventEnd = sortedEvents[sortedEvents.length - 1]?.end?.dateTime
  ...
  return {
    ...
    averageEventDurationMinutes:
      timedEvents.length > 0 ? Math.round(totalMinutes / timedEvents.length) : 0,
    ...
    backToBackCount,
    averageGapMinutes: gapCount > 0 ? Math.round(totalGapMinutes / gapCount) : 0,
