...
const RESOURCE_TABLE_MAP: Record<OwnedResourceType, { table: ValidTableName; userIdColumn: string }> = {
...
export async function validateResourceOwnership(
  ...
  try {
    ...
    const ownerUserId = (data as unknown as Record<string, string>)?.[mapping.userIdColumn]
    if (ownerUserId !== userId) {
      logger.warn(`SECURITY: BOLA attempt - User ${userId} tried to access ${resourceType}:${resourceId} owned by ${ownerUserId}`)
...
export async function requireOwnership(
  ...
  const result = await validateResourceOwnership(userId, resourceType, resourceId)

  if (!result.isOwner) {
    // 404 for "not found" to avoid leaking resource existence, 403 for ownership failures
    const status = result.error === "Resource not found" ? STATUS_RESPONSE.NOT_FOUND : STATUS_RESPONSE.FORBIDDEN
...
export function requireEmailOwnership(req: Request, res: Response, email: string): boolean {
  ...
  if (userEmail.toLowerCase() !== email.toLowerCase()) {
    logger.warn(`SECURITY: Email mismatch - User ${userEmail} tried to access data for ${email}`)
...
export function requireUserIdOwnership(req: Request, res: Response, targetUserId: string): boolean {
  const userId = req.user?.id
  ...
  if (userId !== targetUserId) {
    logger.warn(`SECURITY: User ID mismatch - User ${userId} tried to access data for ${targetUserId}`)
    sendR(res, STATUS_RESPONSE.FORBIDDEN, "Access denied")
    return false
  }
