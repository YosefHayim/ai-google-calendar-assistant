...
export const initWhatsApp = (): void => {
  const { phoneNumberId, accessToken, verifyToken, appSecret, apiVersion } = env.integrations.whatsapp

  if (!isWhatsAppConfigured()) {
    logger.info("WhatsApp: Integration not configured (missing required env vars)")
  ...
  logger.info(`WhatsApp: Signature verification: ${appSecret ? "Enabled" : "Disabled (configure WHATSAPP_APP_SECRET for production)"}`)
}
