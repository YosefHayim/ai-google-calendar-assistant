...
export const downloadMedia = async (mediaId: string): Promise<MediaDownloadResult> => {
  ...
  if (!urlResult.success || !urlResult.url) {
    return { success: false, error: urlResult.error ?? "Failed to get media URL" }
  }
  ...
  try {
    ...
    if (!response.ok) {
      logger.error(`WhatsApp: Failed to download media - Status ${response.status}`)
    ...
    logger.info(`WhatsApp: Downloaded media ${mediaId} - ${buffer.length} bytes`)
...
export const downloadVoiceMessage = async (
  mediaId: string
): Promise<{ success: boolean; audioBuffer?: Buffer; mimeType?: string; error?: string }> => {
