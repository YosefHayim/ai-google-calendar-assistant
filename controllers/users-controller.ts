...
const generateAuthGoogleUrl = reqResAsyncHandler(async (req, res) => {
  ...
  if (!code) {
    ...
    if (postmanHeaders?.includes("Postman")) return sendR(STATUS_RESPONSE.SUCCESS, url);
  ...
  try {
    ...
    if (error) {
      ...
      return sendR(STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to store new tokens.", error);
    }

    sendR(STATUS_RESPONSE.SUCCESS, "Existing token is still valid.", { data });
  } catch (error) {
    console.error("generateAuthUrl error:", error);
    sendR(STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to process OAuth token exchange.", error);
  }
});

const signUpUserReg = asyncHandler(async (req: Request, res: Response) => {
  if (!req.body.email || !req.body.password) sendR(STATUS_RESPONSE.BAD_REQUEST, "Email and password are required.");
  ...
  if (error) {
    ...
    sendR(STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to sign up user.", error);
  }
  if (data) {
    sendR(STATUS_RESPONSE.SUCCESS, "User signed up successfully.", data);
  }
});
...
const getUserInformation = asyncHandler(async (req, res) => {
  if (!req.user) return sendR(STATUS_RESPONSE.UNAUTHORIZED, "User not authenticated.");
  sendR(STATUS_RESPONSE.SUCCESS, "User fetched successfully.", req.user);
});
...
const signInUserReg = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) {
    ...
    sendR(STATUS_RESPONSE.INTERNAL_SERVER_ERROR, "Failed to fetch user by email.", error);
  }

  sendR(STATUS_RESPONSE.SUCCESS, "User signin successfully.", data);
});
