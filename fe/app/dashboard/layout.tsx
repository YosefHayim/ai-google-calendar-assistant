...
export default function DashboardLayout({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      ...
      <main className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? "md:ml-64" : "md:ml-20"}`}>{children}</main>
      <SettingsModal isOpen={isSettingsOpen} onClose={closeSettings} onSignOut={handleSignOut} isDarkMode={isDarkMode} toggleTheme={toggleTheme} />
    </div>
  );
}
