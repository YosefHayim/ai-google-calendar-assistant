            ...
            >
              <Card
                className={`cursor-pointer transition-all duration-200 hover:shadow-md ${
                  selectedPainPoints.includes(painPoint.id) ? 'bg-primary/5 ring-2 ring-primary' : 'hover:bg-muted/50'
                }`}
                ...
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="mb-1 text-lg font-semibold">{painPoint.title}</h3>
                      <p className="mb-3 text-muted-foreground">{painPoint.description}</p>
                      <div className="text-sm text-muted-foreground">
                        <span className="font-medium">We'll help with:</span>
                        <div className="mt-2 flex flex-wrap gap-2">
                          {painPoint.solutions.map((solution) => (
                            <span key={solution} className="rounded-full bg-muted px-2 py-1 text-xs">
                              {solution}
                            </span>
                          ))}
                        </div>
