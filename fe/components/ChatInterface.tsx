...
const AssistantAvatar: React.FC<AssistantAvatarProps> = ({ isRecording, isSpeaking, isLoading, compact }) => {
  return (
    <div className={`flex flex-col items-center justify-center transition-all duration-700 ${compact ? 'scale-75 md:scale-90' : 'h-full w-full animate-in zoom-in'}`}>
      <div className={`relative flex items-center justify-center ${compact ? 'w-[200px] h-[200px] md:w-[300px] md:h-[300px]' : 'w-[280px] h-[280px] sm:w-[320px] sm:h-[320px] md:w-[450px] md:h-[450px]'}`}>
        <div className={`absolute inset-0 rounded-full bg-primary/20 blur-[100px] transition-all duration-1000 ${isSpeaking || isLoading ? 'scale-125 opacity-40' : 'scale-100 opacity-10'}`} />
      ...
      <div className={`${compact ? 'mt-4' : 'mt-8'} text-center relative z-10 px-4`}>
        <div className={`${compact ? 'text-xl' : 'text-3xl md:text-4xl'} font-medium text-zinc-900 dark:text-zinc-100 tracking-tight flex flex-col items-center justify-center gap-2`}>
...
const ComingSoonPlaceholder: React.FC = () => (
  <div className="flex flex-col items-center justify-center h-full w-full animate-in fade-in zoom-in-95 duration-500 text-center px-6">
    <div className="w-24 h-24 bg-zinc-100 dark:bg-zinc-900 rounded-3xl flex items-center justify-center mb-8 relative">
        ...
        <div className="absolute -top-2 -right-2 bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg">NEW</div>
    </div>
...
const ChatInterface: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: 'assistant', content: "Good morning. I've reviewed your schedule; you are clear until 11 AM. Shall I block that time for deep work?", timestamp: new Date() }
  ]);
  ...
  const handleSend = useCallback(async (e?: React.FormEvent, textToSend: string = input) => {
    e?.preventDefault();
    ...
    try {
      ...
      const assistantMessage: Message = { id: (Date.now() + 1).toString(), role: 'assistant', content: responseText || "I encountered an issue processing that instruction.", timestamp: new Date() };
            ...
            <div className="flex-1 space-y-6">
              {messages.map((msg) => (
                <div key={msg.id} className="animate-in fade-in slide-in-from-right-2 duration-300 flex flex-col">
                  <div className={`p-3 rounded-xl text-xs leading-relaxed max-w-[90%] shadow-sm ${msg.role === 'assistant' ? 'bg-zinc-100 dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200 ml-0 mr-auto rounded-tl-none' : 'bg-primary text-white ml-auto mr-0 rounded-tr-none'}`}>
                    {msg.content}
                  </div>
  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto w-full relative overflow-hidden">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 flex bg-white/80 dark:bg-zinc-800/80 backdrop-blur-md p-1 rounded-full shadow-lg border border-zinc-200 dark:border-zinc-700">
        <button onClick={() => setActiveTab('chat')} className={`flex items-center gap-2 p-1.5 px-4 rounded-full text-xs font-bold transition-all ${activeTab === 'chat' ? 'bg-zinc-900 dark:bg-zinc-100 shadow-sm text-white dark:text-zinc-900' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>
          <MessageSquare className="w-3.5 h-3.5" /> Chat
        </button>
        <button onClick={() => setActiveTab('avatar')} className={`flex items-center gap-2 p-1.5 px-4 rounded-full text-xs font-bold transition-all ${activeTab === 'avatar' ? 'bg-zinc-900 dark:bg-zinc-100 shadow-sm text-white dark:text-zinc-900' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>
          <User className="w-3.5 h-3.5" /> 2D
        </button>
        <button onClick={() => router.push('/dashboard/analytics')} className={`flex items-center gap-2 p-1.5 px-4 rounded-full text-xs font-bold transition-all text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300`}>
          <BarChart2 className="w-3.5 h-3.5" /> Intelligence
        </button>
        <button onClick={() => setActiveTab('3d')} className={`flex items-center gap-2 p-1.5 px-4 rounded-full text-xs font-bold transition-all ${activeTab === '3d' ? 'bg-zinc-900 dark:bg-zinc-100 shadow-sm text-white dark:text-zinc-900' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>
      ...
      {/* Input Section */}
      <div id="tour-chat-input" className="absolute bottom-0 left-0 right-0 p-4 z-20 bg-gradient-to-t from-white dark:from-zinc-950 via-white/80 to-transparent">
        {isRecording ? (
          ...
          <form onSubmit={handleSend} className="relative flex items-center bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl p-2 gap-2">
            <button
              type="button"
              onClick={handleToggleRecording}
              className={`p-3 rounded-xl transition-all ${isRecording ? 'text-red-500 bg-red-50' : 'text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100'}`}
            ...
            <input 
              ...
              disabled={isLoading} 
            />
            <button type="submit" disabled={!input.trim() || isLoading} className={`p-3 rounded-xl transition-all ${input.trim() && !isLoading ? 'bg-zinc-950 dark:bg-zinc-100 text-white dark:text-zinc-950' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400'}`}>
