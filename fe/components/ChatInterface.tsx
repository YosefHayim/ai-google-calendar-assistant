    ...
    >
      <div
        className={`relative flex items-center justify-center ${
          compact ? "w-[200px] h-[200px] md:w-[300px] md:h-[300px]" : "w-[280px] h-[280px] sm:w-[320px] sm:h-[320px] md:w-[450px] md:h-[450px]"
        }`}
      >
        <div
...
const ChatInterface: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: "Good morning. I've reviewed your schedule; you are clear until 11 AM. Shall I block that time for deep work?",
      timestamp: new Date(),
    },
  ]);
    ...
    if (msg.role === "assistant") {
      return (
        <div className="flex items-center gap-2 mt-1 px-1">
          <button onClick={() => speakText(msg.content)} className="p-1 text-zinc-400 hover:text-primary transition-colors" title="Hear response">
    ...
    return (
      <div className="flex items-center justify-end gap-3 mt-1.5 px-1">
        ...
        <div className="flex items-center gap-2 border-l border-zinc-200 dark:border-zinc-800 pl-2">
          <button onClick={() => handleResend(msg.content)} className="p-1 text-zinc-400 hover:text-primary transition-colors" title="Reset / Re-trigger">
            <RotateCcw className="w-3 h-3" />
          </button>
          <button onClick={() => speakText(msg.content)} className="p-1 text-zinc-400 hover:text-primary transition-colors" title="Hear message">
            <Volume2 className="w-3 h-3" />
          </button>
          <button onClick={() => handleEditMessage(msg.content)} className="p-1 text-zinc-400 hover:text-primary transition-colors" title="Edit & Resend">
  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto w-full relative overflow-hidden">
      ...
      {/* Main Content Area */}
      <div className="flex-1 relative">{activeTab === "avatar" ? avatarView : activeTab === "3d" ? threeDView : chatView}</div>
      ...
      >
        {isRecording ? (
          <div className="relative flex flex-col items-center justify-center bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl p-4 transition-all">
            ...
            />
            <button onClick={handleCancelRecording} className="absolute top-4 right-4 p-2 rounded-full text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800">
              <X className="w-5 h-5" />
            </button>
          </div>
        ) : (
          ...
          >
            ...
            <button
              ...
              className={`p-3 rounded-xl transition-all ${
                input.trim() && !isLoading ? "bg-zinc-950 dark:bg-zinc-100 text-white dark:text-zinc-950" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400"
              }`}
            >
