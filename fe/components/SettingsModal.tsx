...
const ToggleRow: React.FC<{ label: string; checked: boolean; onChange: (checked: boolean) => void; description?: string }> = ({ label, checked, onChange, description }) => {
  ...
  return (
      <div className="flex items-center justify-between py-4 border-b border-zinc-100 dark:border-zinc-800 last:border-b-0">
          <div className="flex flex-col pr-4">
              <label htmlFor={id} className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 cursor-pointer">{label}</label>
              {description && <p id={`${id}-description`} className="text-xs text-zinc-500 mt-1 leading-relaxed">{description}</p>}
          </div>
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSignOut, isDarkMode, toggleTheme }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-zinc-950 w-full max-w-2xl h-[600px] rounded-xl shadow-2xl flex overflow-hidden border border-zinc-200 dark:border-zinc-800 animate-in zoom-in-95 duration-200">
        <div className="w-56 bg-zinc-50 dark:bg-zinc-900/50 border-r border-zinc-200 dark:border-zinc-800 flex flex-col p-4">
          ...
          <nav className="flex-1 space-y-1">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as Tab)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === tab.id ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 shadow-sm' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800/70'}`}>
                <tab.icon className="w-4 h-4" /> {tab.label}
              </button>
            ))}
          </nav>
          <button onClick={onSignOut} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-red-500 text-sm font-medium mt-auto hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
             ...
             {activeTab === 'general' && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="flex items-center gap-4 p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                    <div className="w-12 h-12 bg-zinc-200 dark:bg-zinc-700 rounded-full flex items-center justify-center text-zinc-500"><User className="w-6 h-6" /></div>
                  ...
                  <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                    <ToggleRow label="Dark Mode" checked={isDarkMode} onChange={() => toggleTheme()} description="Switch between light and high-contrast dark themes." />
                  </div>
                </div>
             )}
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300 pb-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-zinc-500 font-medium">4 synced sources</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 p-1 px-2 rounded-full text-[10px] font-bold border border-green-100 dark:border-green-900/30">
                      ...
                      <button className="w-full mt-2 flex items-center justify-center gap-2 p-2 rounded-lg text-xs font-bold border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-white dark:hover:bg-zinc-800 transition-colors">
                        Manage Calendars <ArrowUpRight className="w-3 h-3" />
                      </button>
                    </div>
                    ...
                    {languages.map((lang) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <span className="text-2xl" role="img" aria-label={lang.region}>{lang.flag}</span>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex bg-white dark:bg-zinc-800 p-1 rounded-lg border border-zinc-200 dark:border-zinc-700">
                          <button 
                            ...
                            className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all ${timeFormat === '12h' ? 'bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900' : 'text-zinc-500'}`}
                          >
                            12H
                          </button>
                          <button 
                            onClick={() => setTimeFormat('24h')}
                            className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all ${timeFormat === '24h' ? 'bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900' : 'text-zinc-500'}`}
                          >
                            24H
                          </button>
                        </div>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-4">
                    <div className="flex items-center gap-3 border-b border-zinc-200 dark:border-zinc-800 pb-4">
                      ...
                      <span className="text-xs font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider">Last Login: <span className="text-zinc-500 dark:text-zinc-400 font-medium ml-2">{lastLogin}</span></span>
                    </div>
                    <ToggleRow 
                  ...
                  <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-4">
                    ...
                    <div className="flex flex-col pt-4 border-t border-zinc-200 dark:border-zinc-800">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Memory Usage: <span className="text-zinc-500 dark:text-zinc-400 font-medium ml-2">{memoryUsage}</span></span>
                      </div>
                      <button onClick={handleClearChatHistory} className="w-full flex items-center justify-center gap-2 p-3 rounded-lg bg-red-500 text-white text-sm font-bold hover:bg-red-600 transition-colors">
