...
function VisaIcon({ className }: { className?: string }) {
  return (
    <svg className={cn('h-8 w-12', className)} viewBox="0 0 48 32" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function PaymentMethodCard({ paymentMethod, onUpdate, className }: PaymentMethodCardProps) {
    ...
    return (
      <Card className={cn('overflow-hidden border border-border', className)}>
        <div className="flex items-center justify-between border-b border-border p-6">
          ...
          {onUpdate && (
            <button onClick={onUpdate} className="text-sm font-medium text-primary hover:text-primary/80">
              Add Card
            </button>
          )}
        </div>
  ...
  const expiryYear = paymentMethod.expiryYear.toString()
    ...
    <Card className={cn('overflow-hidden border border-border', className)}>
      ...
      <div className="flex items-center gap-4 p-6">
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">•••• •••• •••• {paymentMethod.last4}</p>
