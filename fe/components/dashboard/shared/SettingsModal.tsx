...
const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const { data: googleCalendarStatus, isLoading: isGoogleCalendarLoading } =
    useGoogleCalendarStatus({
      enabled: isOpen,
    });

  const { mutate: disconnectGoogleCalendar, isPending: isDisconnecting } =
    useDisconnectGoogleCalendar();
  const {
  ...
  const handleDeleteAllConversations = () => {
    if (
      !window.confirm(
        "Are you sure you want to delete ALL chat logs? This cannot be undone."
      )
    ) {
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 [&>button]:hidden">
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Manage your Ally preferences and settings.
          </DialogDescription>
        </DialogHeader>
