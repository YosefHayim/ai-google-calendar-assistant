...
import { WhatsAppIcon, TelegramIcon } from '@/components/shared/Icons';
...
const FEATURES = [
  ...
  },
  {
    ...
    color: 'text-emerald-500',
    content: (
      <div className="space-y-6 flex flex-col items-center w-full pt-20">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6 relative z-10">
          <div className="flex flex-col gap-4">
            <AnimatePresence mode="wait">
              ...
              >
                <div className={`w-12 h-12 rounded-xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center shadow-sm ${FEATURES[active].color}`}>
            ...
            <div className="flex gap-2 flex-wrap max-w-xs">
              {FEATURES.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setActive(i)}
                  className={`h-1.5 rounded-full transition-all duration-500 mb-2 ${
                    i === active ? 'w-8 bg-primary' : 'w-2 bg-zinc-200 dark:border-zinc-700'
                  }`} 
                />
              ))}
            </div>
