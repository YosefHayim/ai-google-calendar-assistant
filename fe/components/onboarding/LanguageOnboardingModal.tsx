...
export const LanguageOnboardingModal: React.FC<LanguageOnboardingModalProps> = ({ isOpen, onClose, onComplete }) => {
  const [selectedLanguages, setSelectedLanguages] = useState<SupportedEventLanguage[]>(['en'])
      ...
      <DialogContent className="max-w-md bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <DialogTitle className="text-xl">Calendar Language Setup</DialogTitle>
              <DialogDescription className="mt-1">Help Ally understand your calendar better</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            Select the language(s) you use when writing your calendar event titles. This helps Ally accurately detect
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <span className="font-medium text-zinc-900 dark:text-zinc-100">{lang.name}</span>
                    <span className="text-zinc-500 dark:text-zinc-400 ml-2 text-sm">{lang.nativeName}</span>
                  </div>
                </button>
              )
            })}
        ...
        <div className="flex gap-3 pt-2">
          <Button variant="outline" onClick={handleSkip} disabled={isPending} className="flex-1">
            Skip (use English)
          </Button>
          <Button onClick={handleSave} disabled={isPending || selectedLanguages.length === 0} className="flex-1">
            {isPending ? (
              <>
