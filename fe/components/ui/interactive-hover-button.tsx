  ...
  ({ text = "Button", Icon, className, dotClassName, hoverContentClassName, ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          "group relative w-auto cursor-pointer overflow-hidden rounded-full border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-2 text-center font-semibold text-zinc-900 dark:text-zinc-100 transition-all duration-300",
          className
        )}
        {...props}
      >
        ...
        <div
          className={cn(
            "absolute inset-0 z-20 flex translate-x-10 items-center justify-center gap-2 text-white opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100",
