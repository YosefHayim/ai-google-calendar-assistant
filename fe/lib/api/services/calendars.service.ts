...
export const calendarsService = {
  ...
  async getColors(): Promise<ApiResponse<Record<string, { background: string; foreground: string }>>> {
    const { data } = await apiClient.get<ApiResponse<Record<string, { background: string; foreground: string }>>>(
      ENDPOINTS.CALENDARS_COLORS,
    )
    return data
  },
  ...
  async getFreeBusy(): Promise<ApiResponse<Record<string, { busy: Array<{ start: string; end: string }> }>>> {
    const { data } = await apiClient.get<ApiResponse<Record<string, { busy: Array<{ start: string; end: string }> }>>>(
      ENDPOINTS.CALENDARS_FREEBUSY,
    )
    return data
  },
