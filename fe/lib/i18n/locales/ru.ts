...
export const ru: TranslationShape = {
  ...
  pricing: {
    ...
    error: {
      ...
      description: 'Не удалось получить актуальную информацию о ценах. Пожалуйста, попробуйте позже.',
    },
  },
  ...
  settings: {
    ...
    realTimeLocationTooltip:
      'При включении Ally использует ваше текущее местоположение для предоставления контекста при создании событий (например, предложения ближайших мест)',
  ...
  onboarding: {
    ...
    steps: {
      welcome: {
        ...
        description: 'Ваш ИИ-помощник по календарю',
        content:
          'Ally помогает вам управлять календарем легко с помощью естественного языка. Просто скажите Ally, что вам нужно, и он позаботится обо всем остальном.',
        audioText:
      ...
      gaps: {
        title: 'Восстановите потерянное время',
        description: 'Найдите и заполните пробелы в расписании',
        content:
          'Ally автоматически идентифицирует неотслеженное время в вашем календаре. Просмотрите эти пробелы и быстро добавьте события для ведения точного учета ваших активностей.',
        audioText:
          'Восстановление пробелов - уникальная функция, которая помогает вам отслеживать, куда уходит ваше время. Я буду идентифицировать периоды, которые не учтены в вашем календаре, чтобы вы могли их заполнить и вести точное расписание.',
      },
      complete: {
        title: 'Вы готовы!',
        description: 'Начните управлять своим календарем',
        content:
          'Вы готовы использовать Ally! Помните, что вы всегда можете получить доступ к настройкам для персонализации своего опыта, подключения дополнительных календарей или изменения настроек уведомлений.',
        audioText:
          'Поздравляем! Вы готовы начать использовать Ally. Если вам когда-нибудь понадобится помощь, просто спросите меня. Я здесь, чтобы сделать управление вашим календарем простым. Давайте начнем!',
      },
    },
  },
  ...
  featureCarousel: {
    ...
    whatsappRelay: {
      ...
      description:
        'Самый популярный в мире мессенджер, теперь ваша исполнительная командная строка. Приватный, быстрый и всегда доступный.',
    },
    executiveDigests: {
  ...
  billing: {
    ...
    trial: {
      ...
      activeDescription:
        'Ваш бесплатный пробный период заканчивается через {{count}} день. Не потеряйте доступ к вашему ИИ-ассистенту.',
      remaining: 'осталось',
