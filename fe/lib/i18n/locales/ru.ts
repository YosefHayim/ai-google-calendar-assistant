...
export const ru: TranslationShape = {
  ...
  about: {
    ...
    problemSubtitle: 'Современные профессионалы теряют часы каждую неделю из-за хаоса в календаре, который они не просили.',
    problemContextSwitch: 'Переключение контекста',
    problemContextSwitchDesc: 'Прыжки между приложениями, вкладками и задачами. Каждое переключение стоит 23 минуты фокуса.',
    problemCalendarChaos: 'Хаос в календаре',
    problemCalendarChaosDesc: 'Двойные бронирования, путаница с часовыми поясами и бесконечная переписка для простой встречи.',
    problemLostHours: 'Потерянные часы',
    problemLostHoursDesc: 'Административная нагрузка крадёт ваши самые продуктивные часы. Время для глубокой работы исчезает.',
    ...
    visionP1: 'Мы создали Ally, потому что верим: ваш календарь должен работать на вас, а не против. Он должен защищать ваши приоритеты, а не просто записывать их.',
    visionP2: 'Та же ИИ-технология, что используется в корпоративных решениях, должна быть доступна каждому предпринимателю, руководителю и профессионалу, который ценит своё время.',
    ...
    impactHoursDesc: 'Перестаньте терять время на планирование. Верните часы каждую неделю для действительно важной работы.',
    ...
    impactGapsDesc: 'Больше не нужно гадать, куда ушло время. Ally находит и помогает заполнить пробелы в вашем календаре.',
    impactVoice: 'Голос в действие',
    impactVoiceDesc: 'От мысли до запланированного события за секунды. Просто говорите естественно, и Ally сделает остальное.',
    ...
    differencePrivacyDesc: 'Ваши данные зашифрованы, никогда не продаются и не используются для обучения ИИ. Ваш личный офис остаётся личным.',
    differenceMultiPlatform: 'Работает там, где вы',
    differenceMultiPlatformDesc: 'Веб, голос, Telegram, WhatsApp. Ally доступен везде, где вы находитесь, как вам удобно общаться.',
    differenceProactive: 'Проактивный, не пассивный',
    differenceProactiveDesc: 'Ally не просто записывает расписание. Он защищает ваше время, предлагает оптимизации и автоматически решает конфликты.',

    ctaBadge: 'Присоединяйтесь к движению',
    ctaTitle: 'Верните своё время',
    ctaSubtitle: 'Перестаньте бороться с календарём. Начните им управлять. Присоединяйтесь к тысячам профессионалов, которые вернули контроль.',
  ...
  sidebar: {
    ...
    quickAddEventDescription: 'Мгновенно создавайте события календаря с помощью естественного языка или голосового ввода',
